...
void Ekran::drawPoints(){
    ...
    for(int i = 0; i < (int)bezier.size(); i++){
        Ekran::putPixel(bezier[i].x, bezier[i].y, 255, 0, 0);
        Ekran::putPixel(bezier[i].x+1, bezier[i].y, 255, 0, 0);
        Ekran::putPixel(bezier[i].x, bezier[i].y+1, 255, 0, 0);
        Ekran::putPixel(bezier[i].x+1, bezier[i].y+1, 255, 0, 0);
    }
    if(bezier.size() > 3){
        if(action == 4){
